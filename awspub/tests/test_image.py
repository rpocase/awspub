...
def test_create__should_allow_partial_registration(s3_bucket_mock):
    ...
    with patch("boto3.client") as bclient_mock:
        ...
        with patch.object(img, "_get") as get_mock, patch.object(img._snapshot, "copy") as copy_mock:
            ...
            with pytest.raises(exceptions.IncompleteSnapshotSetException):
...
def test_register_images__should_return_none_on_permission_failures():
    instance = MagicMock()
    ...
    assert img._register_images(snapshot_ids, "eu-central-1", instance) is None


def test_register_images__should_raise_on_unhandled_client_error():
    instance = MagicMock()
    ...
    with pytest.raises(botocore.exceptions.ClientError):
        img._register_images(snapshot_ids, "eu-central-1", instance) is None
